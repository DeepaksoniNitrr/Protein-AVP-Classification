# Protein-AVP-Classification
Codes for Sonified Protein AVP Classification
# Sonified Protein
AVP Classification using Signal Processing and Deep Learning

This repository contains the implementation used in the paper:

Deepak Kumar Soni, Dr. Deepak Singh,Meetesh Nevendra  
Attention-Based Deep Learning for Sonified Protein
AVP Classification

Models:
- ENNAVIA A – InceptionV3 
- ENNAVIA B – InceptionV3 + SE Attention
- ENNAVIA C – ResNet50 + CBAM
- ENNAVIA D – ResNet50 + CBAM

- The notebooks contain preprocessing, spectrogram generation, model training and evaluation.
